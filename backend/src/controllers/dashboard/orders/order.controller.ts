import { Body, Controller, DefaultValuePipe, Delete, Get, HttpStatus, Logger, Param, ParseIntPipe, Post, Put, Query, Render, Req, Res, UseGuards } from '@nestjs/common';
...
export class DashboardOrderController {
  ...
  async index(
    ...
    try {
      ...
      let [orders, count ] = await this.orderModel.findAndCount({ skip: (queryPage - 1) * (queryPerPage + 1), take: queryPerPage, relations: ['user'] });

      // Send the paginated list of orders as a JSON response
      return res.status(HttpStatus.OK).json({ orders, count });
  ...
  async view(
    ...
    try {
      ...
      // Fetch the order with the specified ID, including its associated user.
      let { num_id: order_number, timelines } = await this.orderModel.findOne({ where: { id: orderId }, relations: ['timelines'], order: { timelines: { created_at: 'DESC' } } });

      // Return the fetched order as a JSON response with a 200 status code.
      return res.status(HttpStatus.OK).json({ order_number, timelines });
  ...
  async delete(
    ...
    try {
      ...
      // Fetch the order with the specified ID, including its associated user.
      let order = await this.orderModel.delete({ id: orderId });
