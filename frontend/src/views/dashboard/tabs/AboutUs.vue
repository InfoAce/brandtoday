<template>
    <div class="tab-pane fade active show px-4" id="privacy" role="tabpanel" aria-labelledby="aboutus-tab">
          <div class="row">
              <div class="col-lg-6 col-md-12 col-xs-12 px-0 mt-4" style="height: 100%;">
                  <QuillEditor v-model:content="about_us" contentType="html" theme="snow"/>
              </div>
  ...
  const about_us = computed({
      get: ()      => $props.company.about_us,
      set: (value) => emit('updateCompany',{ about_us: value })
  });
  ...
  const saveChanges = async () => {
      try {
          ...
          let { data } = await $api.put('/dashboard/website/about_us', { about_us: about_us.value });
