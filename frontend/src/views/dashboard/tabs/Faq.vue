<template>
    <div class="tab-pane fade active show px-4" id="privacy" role="tabpanel" aria-labelledby="faq-tab">
          <div class="row">
              <div class="col-lg-6 col-md-12 col-xs-12 px-0 mt-4" style="height: 100%;">
                  <QuillEditor v-model:content="faqs" contentType="html" theme="snow"/>
              </div>
  ...
  // Computed values
  const faqs = computed({
      get: ()      => $props.company.faqs,
      set: (value) => emit('updateCompany',{ faqs: value })
  });
  ...
  const saveChanges = async () => {
      try {
          ...
          let { data } = await $api.put('/dashboard/website/faqs', { faqs: faqs.value });
