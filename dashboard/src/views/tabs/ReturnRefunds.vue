<template>
    <div class="tab-pane fade active show px-4" id="privacy" role="tabpanel" aria-labelledby="returnrefunds-tab">
          <div class="row">
              <div class="col-lg-6 col-md-12 col-xs-12 px-0 mt-4" style="height: 100%;">
                  <QuillEditor v-model:content="return_refunds" contentType="html" theme="snow"/>
              </div>
  ...
  // Computed values
  const return_refunds = computed({
    get: ()      => $props.company.return_refunds,
    set: (value) => emit('updateCompany',{ return_refunds: value })
  });
  ...
  const saveChanges = async () => {
      try {
          ...
          let { data } = await $api.put('/dashboard/website/return_refunds', { return_refunds: return_refunds.value });
